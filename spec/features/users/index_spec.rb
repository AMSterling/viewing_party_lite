...
RSpec.describe 'the landing page' do
  it 'shows a list of all users as links to their dashboard' do
    ...
    within "#user-#{user1.id}" do
      expect(page).to have_link('Geraldo', href: user_path(user1))
    end

    within "#user-#{user2.id}" do
      expect(page).to have_link('Maury', href: user_path(user2))
    end

    within "#user-#{user3.id}" do
      expect(page).to have_link('Jenny', href: user_path(user3))
    end
